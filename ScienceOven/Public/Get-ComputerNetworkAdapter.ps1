function Get-ComputerNetworkAdapter {
    <#
    .SYNOPSIS
    Returns NetworkAdapter data for a specific computer

    .DESCRIPTION
    Returns NetworkAdapter data for a specific computer

    .NOTES
    The attributes given here are influenced by the columns in the "NetworkAdapter" tab of the "SQL Server Inventory - Windows" workbook.
    ...
    .EXAMPLE
    Get-ComputerNetworkAdapter -Computer localhost
    ...
    process {
        try {
            ...
            foreach ($c in $Computer) {
                ...
                $CIMNetworkAdapterConfiguration = Get-CimInstance -CimSession $CIMSession -Namespace $NameSpace -ClassName 'Win32_NetworkAdapterConfiguration'

                foreach ($Item in $CIMNetworkAdapterConfiguration) {
                    ...
                    $Props = [PSCustomObject] @{
                        ...
                        'Default Gateway' = $Item.'DefaultIPGateway'
                        'Description'     = $Item.'Description'
                        'DHCP Enabled'    = $Item.'DHCPEnabled'
                        'DHCP Server'     = $Item.'DHCPServer'
                        'DNS Domain'      = $Item.'DNSDomain'
                        'DNS Host Name'   = $Item.'DNSHostName'
                        'DNS Servers'     = $Item.'DnsServerSearchOrder'
                        'IP Addresses'    = $Item.'IPAddress'
                        'IP Subnets'      = $Item.'IPSubnet'
                        'MAC Address'     = $Item.'MACAddress'
                        'WINS Primary'    = $Item.'WINSPrimaryServer'
                        'WINS Secondary'  = $Item.'WINSSecondaryServer'
                    }
                    $Props
                }
            }
