function Get-ComputerLocalUser {
    <#
    .SYNOPSIS
    Returns LocalUser data for a specific computer

    .DESCRIPTION
    Returns LocalUser data for a specific computer

    .NOTES
    The attributes given here are influenced by the columns in the "LocalUser" tab of the "SQL Server Inventory - Windows" workbook.
    ...
    .EXAMPLE
    Get-ComputerLocalUser -Computer localhost
    ...
    process {
        try {
            ...
            foreach ($c in $Computer) {
                ...
                $CIMLocalUser = Get-CimInstance -CimSession $CIMSession -Namespace $NameSpace -ClassName 'Win32_UserAccount'

                foreach ($Item in $CIMLocalUser) {
                    ...
                    $Props = [PSCustomObject] @{
                        ...
                        'Locked Out'          = $Item.'Lockout'
                        'Password Changeable' = $Item.'PasswordChangeable'
                        'Password Expires'    = $Item.'PasswordExpires'
                        'Password Required'   = $Item.'PasswordRequired'
                        'Username'            = $Item.'Name'
                    }
                    $Props
                }
