function Get-ComputerShare {
    <#
    .SYNOPSIS
    Returns Share data for a specific computer

    .DESCRIPTION
    Returns Share data for a specific computer

    .NOTES
    The attributes given here are influenced by the columns in the "Share" tab of the "SQL Server Inventory - Windows" workbook.
    ...
    .EXAMPLE
    Get-ComputerShare -Computer localhost
    ...
    process {
        try {
            ...
            foreach ($c in $Computer) {
                ...
                $CIMShare = Get-CimInstance -CimSession $CIMSession -Namespace $NameSpace -ClassName 'Win32_Share'

                foreach ($Item in $CIMShare) {
